# Go API client for edge/api/openapi
...
```go
import edge/api/openapi "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `edge/api/openapi.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), edge/api/openapi.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `edge/api/openapi.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), edge/api/openapi.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `edge/api/openapi.ContextOperationServerIndices` and `edge/api/openapi.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), edge/api/openapi.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), edge/api/openapi.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), edge/api/openapi.ContextAccessToken, "BEARER_TOKEN_STRING")
...
auth := context.WithValue(
		context.Background(),
		edge/api/openapi.ContextAPIKeys,
		map[string]edge/api/openapi.APIKey{
